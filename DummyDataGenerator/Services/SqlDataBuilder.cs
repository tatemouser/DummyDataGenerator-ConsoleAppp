...
namespace DummyDataGenerator.Services
{
    public class SqlDataBuilder
    {
        ...
        public HashSet<string> KnownTemplates { get; } = new()
        {
        ...
        public string GetValueForType(string templateName, int rowIndex)
        {
            switch (templateName)
            {
        ...
        public List<Dictionary<string, string>> BuildInMemory(List<FieldDefinition> fields, int rowCount)
            {
                ...
                for (int i = 0; i < rowCount; i++)
                {
                    ...
                    foreach (var field in fields)
                    {
                        string value = KnownTemplates.Contains(field.Name.ToLower())
                            ? GetValueForType(field.Name.ToLower(), i)
